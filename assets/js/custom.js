...
function downloadButton(folder, file, use_nama = true){
  if(file)
    return `<a class="btn btn-success btn-xs btn-download" href="${folder}${file}"><i class='fa fa-download'></i> ${use_nama ? file : 'Download'}</a>`;
  return `Tidak Ada`;
}

function downloadButtonV2(folder, file, override_nama){
  if(file)
    return `<a class="btn btn-success btn-xs btn-download" href="${folder}${file}"><i class='fa fa-download'></i> ${override_nama ? override_nama : file}</a>`;
  return `Tidak Ada`;
}
...
function renderBreadcrumb(data){
  ...
  breadcrumb.empty();
  data.forEach((e) => {
    if(e[0]) breadcrumb.append(`<li class="breadcrumb-item"><a href="${e[0]}">${e[1]}</a></li>`);
    else breadcrumb.append(`<li class="breadcrumb-item active"><strong>${e[1]}</strong></li>`);
  });
}