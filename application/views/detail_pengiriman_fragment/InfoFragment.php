...
$(document).ready(function() {
  ...
  pengiriman_item_modal.tanggal_pengiriman.datepicker({todayBtn: "linked", autoclose: true, format: "yyyy-mm-dd"});
  ...
  swal({title: 'Loading harga_mwp...', allowOutsideClick: false});
  swal.showLoading();
  ...
  function getAllJenisMutu(){
    return $.ajax({
      url: `<?php echo site_url('JenisMutuController/getAll/')?>`, 'type': 'GET',
      data: {},
  ...
  function renderAllJenisMutu(data){
    ...
    pengiriman_item_modal.id_jenis_mutu.append($('<option>', { value: "", text: "-- Pilih Mutu --"}));
  ...
  function getAllNegara(){
    return $.ajax({
      url: `<?php echo site_url('ParameterController/getAllNegara/')?>`, 'type': 'GET',
      data: {},
      success: function (data){
        var json = JSON.parse(data);
        if(json['error']){
  ...
  function renderAllNegara(data){
    ...
    pengiriman_item_modal.id_negara.append($('<option>', { value: "", text: "-- Pilih Negara --"}));
    Object.values(data).forEach((d) => {
  ...
  function getAllPortOfOrigin(){
    return $.ajax({
      url: `<?php echo site_url('PortOfOriginController/getAll/')?>`, 'type': 'GET',
      data: {},
      success: function (data){
        var json = JSON.parse(data);
        if(json['error']){
  ...
  function renderAllPortOfOrigin(data){
    ...
    pengiriman_item_modal.id_port_of_origin.append($('<option>', { value: "", text: "-- Pilih Port Of Origin --"}));
    Object.values(data).forEach((d) => {
  ...
  function getAllJenisPengemasan(){
    return $.ajax({
      url: `<?php echo site_url('JenisPengemasanController/getAll/')?>`, 'type': 'GET',
      data: {},
      success: function (data){
        var json = JSON.parse(data);
        if(json['error']){
  ...
  function renderAllJenisPengemasan(data){
    ...
    pengiriman_item_modal.id_jenis_pengemasan.append($('<option>', { value: "", text: "-- Pilih Jenis Pengemasan --"}));
    Object.values(data).forEach((d) => {
  ...
  DokPermohonanBpsmb.form.on('submit', (ev) => {
    ...
    $.ajax({
      url: "<?=site_url('PengirimanController/dok_permohonan_upload_bpsmb')?>", type: "POST",
  ...
  informasiModal.form.on('submit', (ev) => {
    ...
    $.ajax({
      url: "<?=site_url('PengirimanController/edit')?>", type: "POST",
  ...
  info.process_btn.on('click', function(){
    ...
    swal(saveConfirmation("Kirim Proposal", "Yakin kirim proposal?", "Ya, Kirim!")).then((result) => {
      ...
      $.ajax({
        url: "<?=site_url('PengirimanController/edit')?>", 'type': 'POST',
        data: {'id_pengiriman': id_pengiriman, 'status_proposal': 'DIPROSES'},
        success: function (data){
          var json = JSON.parse(data);
          if(json['error']){
  ...
  info.delete_btn.on('click', function(){
    swal(deleteConfirmation("Konfirmasi hapus", "Yakin akan menghapus pengiriman ini?", "Ya, Hapus!")).then((result) => {
      if(!result.value){ return; }
      $.ajax({
        url: "<?=site_url('PengirimanController/delete')?>", 'type': 'POST',
  ...
  function getAllPengirimanItem(){
    ...
    return $.ajax({
      url: `<?php echo site_url('PengirimanItemController/getAll/')?>`, 'type': 'GET',
      data: {'id_pengiriman': id_pengiriman},
  ...
  pengiriman_item_table.on('click','.delete', function(){
    ...
    swal(deleteConfirmation("Konfirmasi hapus", "Yakin akan menghapus item ini?", "Ya, Hapus!")).then((result) => {
      if(!result.value){ return; }
      $.ajax({
        url: "<?=site_url('PengirimanItemController/delete')?>", 'type': 'POST',
        data: {'id_pengiriman_item': id},
        success: function (data){
          var json = JSON.parse(data);
          if(json['error']){
            swal("Delete Gagal", json['message'], "error");
