...
$(document).ready(function() {
  ...
  pengiriman_datatable.on('click', '.duplikat', function(){
   ...
   $.ajax({
      url: "<?=site_url('PengirimanController/duplikat')?>", 'type': 'Get',
      data: {id_pengiriman: currentData['id_pengiriman']},
      success: function (data){
  ...
  toolbar.newBtn.on('click', function(e){
    ...
    $.ajax({
      url: "<?=site_url('PerusahaanController/addPengiriman')?>", 'type': 'POST',
      data: {id_perusahaan: id_perusahaan},
      success: function (data){
        buttonIdle(toolbar.newBtn);
        var json = JSON.parse(data);
