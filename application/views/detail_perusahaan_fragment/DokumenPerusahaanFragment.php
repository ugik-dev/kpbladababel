...
$(document).ready(function() {
  ...
  var dokumen_perusahaan_modal = {
    ...
    'dokumen_perusahaan': new FileUploader($('#dokumen_perusahaan_modal').find('#dokumen_perusahaan'), "", "dokumen_perusahaan", ".pdf", false, true),
  }
  ...
  function getAllDokumenPerusahaan(){
    swal({title: 'Loading pengiriman...', allowOutsideClick: false});
    swal.showLoading();
    return $.ajax({
      url: `<?php echo site_url('DokumenPerusahaanController/getAll/')?>`, 'type': 'GET',
  ...
  function renderDokumenPerusahaan(data){
    ...
    Object.values(data).forEach((dokumen) => {
      ...
      `;
      renderData.push([dokumen['nama_jenis_dokumen_perusahaan'], dokumen['no_dokumen_perusahaan'], downloadButton("<?=site_url('uploads/dokumen_perusahaan/')?>", dokumen['dokumen_perusahaan'], true), button]);
    });
  ...
  function getAllJenisDokumenPerusahaan(){
    return $.ajax({
      url: `<?php echo site_url('ParameterController/getAllJenisDokumenPerusahaan/')?>`, 'type': 'GET',
      data: {},
      success: function (data){
        var json = JSON.parse(data);
        if(json['error']){
  ...
  function renderJenisDokumenPerusahaan(data){
    ...
    dokumen_perusahaan_modal.id_jenis_dokumen_perusahaan.append($('<option>', { value: "", text: "-- Pilih Jenis Dokumen Perusahaan --"}));
  ...
  dokumen_perusahaan_modal.form.submit(function(event) {
    ...
    swal(saveConfirmation("Konfirmasi tambah", "Yakin akan menambahakan dokumen ini?", "Ya, Tambah!")).then((result) => {
      ...
      $.ajax({
        url: "<?=site_url('DokumenPerusahaanController/add')?>", 'type': 'POST',
  ...
  dokumen_perusahaan_datatable.on('click','.delete', function(){
    ...
    swal(deleteConfirmation("Konfirmasi hapus", "Yakin akan menghapus dokumen ini?", "Ya, Hapus!")).then((result) => {
      if(!result.value){ return; }
      $.ajax({
        url: "<?=site_url('DokumenPerusahaanController/delete')?>", 'type': 'POST',
        data: {'id_dokumen_perusahaan': id},
        success: function (data){
          var json = JSON.parse(data);
          if(json['error']){
