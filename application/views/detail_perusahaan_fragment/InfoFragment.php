...
$(document).ready(function() {
  ...
  function getAllJenisPerusahaan(){
    return $.ajax({
      url: `<?php echo site_url('ParameterController/getAllJenisPerusahaan/')?>`, 'type': 'GET',
      data: {},
  ...
  function renderJenisPerusahaan(data){
    ...
    informasiModal.id_jenis_perusahaan.append($('<option>', { value: "", text: "-- Pilih --"}));
  ...
  function getAllBank(){
    return $.ajax({
      url: `<?php echo site_url('BankController/getAll/')?>`, 'type': 'GET',
      data: {},
      success: function (data){
        var json = JSON.parse(data);
        if(json['error']){
  ...
  informasiModal.form.on('submit', (ev) => {
    ...
    $.ajax({
      url: "<?=site_url('PerusahaanController/update')?>", type: "POST",
