...
$(document).ready(function () {
  ...
  initNavbar(200);
  $.when(getLatestHargaMWP(),getStandarMutu(), getAllProduct(), getAllNews()).then((e) =>{
  ...
  function getLatestHargaMWP(){
    ...
    swal.showLoading();
    return $.ajax({
      url: `<?php echo site_url('HargaMWPController/getLatest/')?>`, 'type': 'GET',
      data: {},
  ...
  function getStandarMutu(){
    swal({title: 'Loading standar_mutu...', allowOutsideClick: false});
    swal.showLoading();
    return $.ajax({
      url: `<?php echo site_url('HargaMWPController/getStandarMutu/')?>`, 'type': 'GET',
      data: {},
      success: function (data){
        swal.close();
        var json = JSON.parse(data);
